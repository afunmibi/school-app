...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!$student_id || !$full_name || !$class) {
        $message = '<div class="alert alert-danger">Please select a student and ensure all fields are filled.</div>';
    } else {
        $stmt = $conn->prepare("INSERT INTO exam_results (student_id, full_name, exam_score, teacher_id, status, class, result_date) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isissss", $student_id, $full_name, $exam_score, $teacher_id, $status, $class, $result_date);
        if ($stmt->execute()) {
...
$students = [];
$sql = "SELECT id, full_name, class FROM students ORDER BY full_name ASC";
                    ...
                    <select name="student_id" id="student_id" class="form-select" required>
                        ...
                        <?php foreach ($students as $student): ?>
                            <option value="<?= $student['id'] ?>"
                                ...
                                <?= htmlspecialchars($student['full_name']) ?> (<?= htmlspecialchars($student['class']) ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
